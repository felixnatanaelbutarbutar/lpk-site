    ...
    <form action="{{ route('pendaftaran.store', ['lang' => app()->getLocale()]) }}" method="POST" enctype="multipart/form-data" class="row g-3">
        ...
        <div class="col-md-6">
            ...
            <select name="pernah_belajar_bahasa_jepang" class="form-control" required>
                <option value="">-- {{ translateText('Pilih') }} --</option>
                <option value="Pernah" {{ old('pernah_belajar_bahasa_jepang') == 'Pernah' ? 'selected' : '' }}>Pernah</option>
                <option value="Tidak Pernah" {{ old('pernah_belajar_bahasa_jepang') == 'Tidak Pernah' ? 'selected' : '' }}>Tidak Pernah</option>
            </select>
        </div>
        ...
        <div class="col-md-6">
            ...
            <select name="pernah_ke_jepang" class="form-control" required>
                <option value="">-- {{ translateText('Pilih') }} --</option>
                <option value="Pernah" {{ old('pernah_ke_jepang') == 'Pernah' ? 'selected' : '' }}>Pernah</option>
                <option value="Tidak Pernah" {{ old('pernah_ke_jepang') == 'Tidak Pernah' ? 'selected' : '' }}>Tidak Pernah</option>
            </select>
        </div>

        <div class="col-md-6">
            <label class="form-label">{{ translateText('Tujuan ke Jepang') }}:</label>
            <input type="text" name="tujuan_ke_jepang" value="{{ old('tujuan_ke_jepang') }}" class="form-control" required>
        </div>

        <div class="col-md-6">
            <label class="form-label">{{ translateText('Sumber Informasi') }}:</label>
            <input type="text" name="sumber_info" value="{{ old('sumber_info') }}" class="form-control" required>
        </div>
